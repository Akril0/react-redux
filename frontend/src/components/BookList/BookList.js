...
import {deleteBook, selectBooks, toggleFavorite} from '../../redux/slices/bookSlice.js';
import {
    selectAuthorFilter, selectOnlyFavorite, selectTitleFilter,
...
const BookList = () => {
    ...
    return (<div className="app-block book-list">
        ...
        {books.length === 0 ? (<p>No books available</p>) : (<ul>
            {filteredBooks.map((book, i) => (<li key={book.id}>
                <div
                    className="book-info">{++i}. {highlightMatch(book.title, titleFilter)} by{' '}
                    <strong>
